...
use super::{
    error::ErrorCode,
    settings::{SettingParam, SettingsIdentifier},
};
...
impl TryFrom<Bytes> for FrameHeader {
    ...
    fn try_from(mut buf: Bytes) -> Result<Self, Self::Error> {
        ...
        let length: usize = ((buf.get_u8() as usize) << 16)
...
impl FrameBody {
    ...
    pub fn size(&self) -> usize {
        match self {
            Self::Data { pad_length, data } => data.len() + pad_length.map_or(0, |n| n + 1),
            Self::Headers {
                pad_length,
                hdr_block_frag,
                priority,
            } => {
                pad_length.map_or(0, |n| n + 1)
                    + hdr_block_frag.len()
                    + priority.as_ref().map_or(0, |_| 5)
            }
            Self::Priority { .. } => 4 + 1,
            Self::RstStream { .. } => 4,
            Self::Settings(settings) => settings.len() * 6,
            Self::PushPromise {
                pad_length,
                hdr_block_frag,
                ..
            ...
            Self::WindowUpdate { .. } => 4,
            Self::Continuation { hdr_block_frag } => hdr_block_frag.len(),
        }
    }
    ...
    fn try_put_buf(&self, buf: &mut BytesMut) -> Result<(), &'static str> {
        match self {
            ...
            Self::Headers {
                ...
                let hdr_block_frag_bytes: Bytes = (*hdr_block_frag)
                    .clone()
                    .try_into()
                    .map_err(|_| "Error compressing header")?;
                ...
                match priority {
                    Some(priority) => {
                        buf.put_u32(if priority.e {
                            priority.stream_dep | 0x8000_0000
                        } else {
                            priority.stream_dep
                        });
    ...
    fn try_from_buf(
        mut buf: Bytes,
        hdr: &FrameHeader,
    ) -> Result<Self, error::BodyDeserializationError> {
        let flags = hdr.flags;
        match hdr.frame_type {
            ...
            0x1 => {
                ...
                let priority: Option<HeadersBodyPriority> =
            ...
            0x3 => {
                ...
                match error_code.try_into() {
                    ...
                    Err(_) => Err(error::BodyDeserializationError::UnknownErrorCode(
                        error_code,
                    )),
                }
            }
            0x4 => {
                ...
                while remaining_size >= 6 {
                    ...
                    let identifier: SettingsIdentifier = identifier.try_into().map_err(|_| {
                        error::BodyDeserializationError::UnknownSettingsIdentifier(identifier)
                    })?;
            ...
            0x5 => {
                ...
                let pad_length = if PushPromiseFlags::from_bits_retain(flags)
                    .contains(PushPromiseFlags::PADDED)
                {
                    let pad_length: usize = buf.get_u8().into();
                    hdr_block_frag_len -= 1 + pad_length;
                    Some(pad_length)
...
impl Frame {
    ...
    /// Deserialization
    pub fn try_read_from_buf(
        buf_reader: &mut BufReader<TcpStream>,
    ) -> Result<Frame, error::DeserializationError> {
        let mut header_buf = BytesMut::zeroed(9);
        // Read frame header
        buf_reader
            .read_exact(header_buf.as_mut())
            .map_err(|_| error::DeserializationError::BufReaderError)?;
        ...
        buf_reader
            .read_exact(payload_buf.as_mut_slice())
            .map_err(|_| error::DeserializationError::BufReaderError)?;
